...
We want to aid creators as they embark on their creative endeavors, whether it be finding the best words to describe their project or determining the best launch day. As crowdfunding continues to grow in popularity, more and people will benefit from analyses like these. 
...
[Laura Lewis](https://medium.com/@lvlewis8), an Amazon data scientist who examined Kickstarter success trends throughout the years, created a [time series plot](https://towardsdatascience.com/using-machine-learning-to-predict-kickstarter-success-e371ab56a743#:~:text=Changes%20over%20time%20in%20the%20number%20of%20projects%20launched%20on%20Kickstarter) with the total number of launched projects each year from 2009-2019. She found that midway through 2014, Kickstarter saw a significant rise in campaigns with a peak of over 4000 projects launched. Thus, the proportion of successful projects decreased as the number of launched projects increased. Nevertheless, the site has seen considerable improvement in success rates in recent years while the number of launched projects have remained fairly constant since 2016. 
...
In 2011, a year prior to the change in campaign length, there is a moderately strong, negative, and roughly linear association between campaign length and success rate. So as the campaign length increases, the success rate decreases. 
...
We begin by splitting our data set into training and testing sets. 70% of the Kickstarter campaigns are put into the training set while the other 30% are put into the testing set. The training set contains 119,238 campaigns and the testing set contains 51,298 campaigns. Splitting the data into training and testing set allows us to see how well our model preforms on an out of sample data set. In other words, it allows us to test our model's predictive ability on data that it has never seen before. 
...
4) `staff_pick` (Categorical, TRUE/FALSE): Whether or not the project was selected by the staff at Kickstarter to be included in the "Projects we Love" section. See [How to Become a Kickstarter “Project We Love”](https://www.crowdcrux.com/become-kickstarter-staff-pick/) to more about getting featured on Kickstarter. 
...
The predictor with the largest significant effect size is staff pick, with a corresponding odds ratio of $\exp(\hat \beta) = \exp(2.62) = 13.7$. This means that the estimated odds of success for a campaign that is a staff pick is 13.7 times higher than a campaign that is not a staff pick, after accounting for the effect of the other predictors. The predictor with the next largest significant effect size is the indicator variable that corresponds to the comic category, with a corresponding odds ratio of $\exp(\hat \beta) = \exp(1.48) = 4.4$. This means that the estimated odds of success for a comic book campaign is 4.40 times higher than a campaign with the baseline category, art, after accounting for the effect of the other predictors.  

## Model Evaluation

We evaluated the model using accuracy, precision, recall, and specificity, which are calculated using the counts of the true positive (TP), true negative (TN), false positive (FP) and false negative (FN) results.
...
Whether you're a prospective Kickstarter creator, data scientist, or curious onlooker, we hope this blog fostered a greater understanding of the factors that go into creating a successful Kickstarter campaign. The biggest takeaway is that creators should do everything they can to get featured in the "Projects We Love" section. Besides having a strong idea and a compelling project image, according to a [blog post](https://www.kickstarter.com/blog/a-new-way-to-explore-an-incredible-creative-universe) on Kickstarter's website, they recommend that creators email stories@kickstarter.com to maximize their chances of getting featured.
