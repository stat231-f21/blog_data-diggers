...
$$Accuracy =  \frac{(TP + TN)}{(TP + TN + FP + FN)} = \frac{(24155 + 12860)}{(24155 + 12860 + 9376 + 4905)} = 0.7216$$

The model is 72.16% accurate, meaning that it predicts the correct outcome 72.16% of the time.

$$Precision =  \frac{(TP)}{(TP + FP)} = \frac{(24155)}{(24155 + 9376)} = 0.7204$$


The model's precision is 72.04%, meaning that 72.04% of the campaigns predicted to be successful were truly successful. In other words, when it predicts a campaign will be successful, it is correct 72.04% of the time.

$$Recall =  \frac{(TP)}{(TP + FN)} = \frac{(24155)}{(24155 + 4905)} = 0.8312$$


The model's recall is 83.12%, meaning that of all the successful campaigns, the model correctly predicted them to be successful 83.12% of the time (true positive rate). In other words, it correctly identifies 83.12% of all successful campaigns. 
